...
export default function ChildMBTI() {
  ...
  function handleAnswer(choice) {
    ...
    let ns = scores;
    if (choice !== "C") {
      const key = choice === "A" ? q.sa : q.sb;
      ns = { ...scores, [key]: scores[key] + 1 };
      setScores(ns);
    }
      ...
      <div style={wrap}>
        <div style={card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {[
              { label: q.a, ch: "A", neutral: false },
              { label: q.b, ch: "B", neutral: false },
              { label: "잘 모르겠어요 🤔", ch: "C", neutral: true },
            ].map(({ label, ch, neutral }) => (
              <button key={ch} onClick={() => handleAnswer(ch)} style={{
                padding: neutral ? "14px 20px" : "18px 20px",
                borderRadius: "18px",
                border: neutral ? `1.5px dashed #CCC` : `2px solid ${c.p}25`,
                background: neutral ? "#FAFAFA" : "white",
                fontSize: bigFont ? (neutral ? "15px" : "17px") : (neutral ? "14px" : "15px"),
                color: neutral ? "#888" : "#444",
                cursor: "pointer", textAlign: neutral ? "center" : "left",
                fontFamily: "inherit", lineHeight: 1.6,
                transition: "all 0.15s ease",
                marginTop: neutral ? "4px" : "0",
              }}
                onMouseEnter={e => {
                  if (neutral) { e.currentTarget.style.background = "#F0F0F0"; e.currentTarget.style.borderColor = "#999"; e.currentTarget.style.color = "#555"; }
                  else { e.currentTarget.style.background = c.bg; e.currentTarget.style.borderColor = c.p; e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = `0 6px 20px ${c.p}25`; }
                }}
                onMouseLeave={e => {
                  if (neutral) { e.currentTarget.style.background = "#FAFAFA"; e.currentTarget.style.borderColor = "#CCC"; e.currentTarget.style.color = "#888"; }
                  else { e.currentTarget.style.background = "white"; e.currentTarget.style.borderColor = `${c.p}25`; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; }
                }}
              >{label}</button>
            ))}
          </div>
