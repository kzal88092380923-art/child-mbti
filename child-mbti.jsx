...
const QUIZ_POOL = [
  ...
  { q: "이효리가 소속된 1990년대 여자 아이돌 그룹은?", hint: "💡 이효리·옥주현·이진·성유리 4인조", a: "핑클" },
  ...
  { q: "2002년 KBS2 드라마, 배용준·최지우 주연의 멜로물은?", hint: "💡 계절 이름 + 연가", a: "겨울연가" },
...
export default function ChildMBTI() {
  ...
  // 슬라이더 트랙 폭 측정
  useEffect(() => {
    if (parentStep !== "slider") return;
    const measure = () => {
      if (sliderTrackRef.current) {
        setSliderTrackWidth(sliderTrackRef.current.offsetWidth);
      }
    };
    measure();
  ...
  // 부모 인증 슬라이더 — touch/pointer 직접 제어 (CSS transition 없이 손가락 그대로 따라옴)
  function unlockSliderToQuiz() {
  ...
  function updateSliderFromX(clientX, rect) {
    const pct = Math.min(100, Math.max(0, ((clientX - rect.left) / rect.width) * 100));
    sliderValRef.current = pct;
    setSliderVal(pct);
  }
  function onSliderPointerDown(e) {
    if (parentStep !== "slider" || !sliderTrackRef.current) return;
    const rect = sliderTrackRef.current.getBoundingClientRect();
    try { e.currentTarget.setPointerCapture(e.pointerId); } catch {}
    sliderDragRef.current = { rect, pointerId: e.pointerId };
    setSliderDragging(true);
    updateSliderFromX(e.clientX, rect);
  }
  function onSliderPointerMove(e) {
    if (!sliderDragRef.current) return;
    e.preventDefault();
    updateSliderFromX(e.clientX, sliderDragRef.current.rect);
  }
  function onSliderPointerEnd(e) {
    if (!sliderDragRef.current) return;
    try { e.currentTarget.releasePointerCapture(sliderDragRef.current.pointerId); } catch {}
    sliderDragRef.current = null;
    setSliderDragging(false);
    const finalVal = sliderValRef.current;
    if (finalVal >= 95) {
      setTimeout(unlockSliderToQuiz, 200);
    } else {
      snapSliderBack();
    }
  }
  ...
  if (screen === "result" && char && pdata) {
    ...
    <div style={{ ...wrap, alignItems: "flex-start", paddingTop: "32px", paddingBottom: "40px" }}>
      <div style={{ ...card, maxWidth: "500px" }} className="pop">
        ...
        {/* 슬라이더 — touch/pointer 직접 제어, CSS transition 없음 */}
        {parentStep === "slider" && (() => {
          const thumbX = Math.max(0, (sliderVal / 100) * Math.max(0, sliderTrackWidth - 44));
          const labelText = sliderVal < 95
            ? (sliderVal < 10 ? "← 밀어주세요" : sliderVal < 50 ? "계속 →" : "거의 다 됐어요! →")
            : "✓";
          return (
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ padding: "8px 0" }}>
                ...
                >
                  {/* Fill — 드래그 중엔 transition 없음 */}
                  <div style={{
                    position: "absolute", top: 0, left: 0,
                    width: `${sliderVal}%`, height: "100%",
                    background: "#EDE9FE",
                    transition: sliderDragging ? "none" : "width 0.32s cubic-bezier(0.4, 0, 0.2, 1)",
                    pointerEvents: "none",
                    willChange: "width",
                  }} />
                  {/* Label */}
                  <div style={{
                    ...
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: "13px", color: sliderVal > 40 ? "#7C3AED" : "#999",
                    fontWeight: 700, pointerEvents: "none",
                    fontFamily: '"Nanum Gothic", sans-serif',
                  }}>{labelText}</div>
                  {/* Thumb — translate3d 로 GPU 가속, 드래그 중엔 transition 없음 */}
                  <div style={{
                    position: "absolute", top: "50%", left: 0,
                    transform: `translate3d(${thumbX}px, -50%, 0)`,
                    ...
                    pointerEvents: "none",
                    transition: sliderDragging ? "none" : "transform 0.32s cubic-bezier(0.4, 0, 0.2, 1)",
        ...
        {parentStep === "open" && (
          <div className="slide-up gpu-layer">
            ...
            <button onClick={kakaoShareFull} style={{
              ...
              background: "linear-gradient(135deg, #7C3AED, #4F46E5)",
              color: "white",
              fontSize: "16px", fontWeight: 800, fontFamily: "inherit", cursor: "pointer",
            ...
            <button onClick={kakaoShareTest} style={{
              ...
              <span style={{ fontSize: "17px" }}>💬</span> 테스트 공유하기
            </button>
