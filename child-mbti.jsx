...
export default function ChildMBTI() {
  useEffect(() => {
    ...
    const shareUrl = window.location.origin + window.location.pathname + "?go=1";
  ...
  function getShareUrl() {
    return window.location.origin + window.location.pathname + "?go=1";
  }
  function getOgImageUrl(type) {
    if (type && CHARS[type]) return window.location.origin + "/og/" + type + ".png";
    return window.location.origin + "/og-image.png";
  }
  function kakaoShare({ title, description, buttonTitle, type }) {
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: "18px" }}>
            <button onClick={kakaoShareTest} style={{
              ...
              background: "#FEE500", color: "#000000d9",
              fontSize: "13px", fontWeight: 800, fontFamily: "inherit", cursor: "pointer",
              boxShadow: "0 3px 10px rgba(254,229,0,0.30)",
            }}>
              <span style={{ fontSize: "15px" }}>💬</span> 친구에게 테스트 공유
            </button>
          </div>
        </div>
      </div>
